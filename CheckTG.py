...
class CheckerTGMod(loader.Module):
	...
	strings = {
		'name': 'CheckerTG',
		'check': '[MUR_API] Делаем запрос к API...'
		}
	def __init__(self):
	...
	async def checkcmd(self, m):
		...
		if utils.get_args_raw(m):
			...
			await m.edit("[MUR_API] А кого чекать?")
			return
		await m.edit(self.strings['check'])
		await m.edit(f"[MUR_API] Ответ API: <code>{requests.get('http://api.murix.ru/eye?uid=' + user).json()['data']}</code>")
	async def scheckcmd(self, m):
		...
		if utils.get_args_raw(m):
			...
			return
		await m.edit(self.strings['check'])
		await m.client.send_message("me", f"[MUR_API] Ответ API: <code>{requests.get('http://api.murix.ru/eye?uid=' + user).json()['data']}</code>")
		await m.edit(f"[MUR_API] Ответ API отправлен в избранное!")
